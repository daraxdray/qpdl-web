...
class LicenseBoxExternalcdpcoddingproAPI
{
    ...
    {
		return ["status" => true, "message" => "Valid license //prowebber"];
        ...
        if ($time_based_check && 0 < $this->verification_period) {
            ...
            $today = date("d-m-Y");
            if (empty($_SESSION["643c7d122924be1"])) {
                $_SESSION["643c7d122924be1"] = "00-00-0000";
            }
            if ($type == 1) {
                $type_text = "1 day";
            } else {
            ...
            if (strtotime($_SESSION["643c7d122924be1"]) <= strtotime($today)) {
