...
if (empty($errors)) {
  ...
  switch ($to) {
    ...
    case "support":
      ...
      // $db->cdp_execute();
      $userEmail = cdp_sanitize($_POST['email']);
      $userName = cdp_sanitize($_POST['full_name']);;
      $userPhone = cdp_sanitize($_POST['phone']);;
      $emailType = cdp_sanitize($_POST['email_type']);;
        
      $email_body = "<html> 
                  
      <body> 
      
      <h4>Fullname</h4>: $userName \n <h4>Email</h4>: $userEmail \n 
      <h4>Phone</h4>: $userPhone <br><br><br><br>
      <p> $body</p>
      ...
      if ($core->mailer == 'PHP') {
        ...
        $mail_send = mail($emailAddress, $subject, $message, $header);
        /*FINALIZA RECOLECTANDO DATOS PARA FUNCION MAIL*/
      } elseif ($core->mailer == 'SMTP') {
        ...
        $mail->From = $userEmail; // Email desde donde envío el correo.
        $mail->FromName = $userName;
        $mail->AddAddress($destinatario); // Esta es la dirección a donde enviamos los datos del formulario

        $mail->Subject = "Support  -". $subject; // Este es el titulo del email.
        $mail->Body = $message; // Texto del email en formato HTML
        // FIN - VALORES A MODIFICAR //
  ...
  if ($mail_send) {
    ...
    $messages[] = "Email(s) have been sent successfully!";
  } else {
