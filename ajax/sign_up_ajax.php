...
if ($value == 2)
    ...
    $errors['username'] = 'Username must alphabets in lower-case.';
...
if (empty($errors)) {
    ...
    $datos = array(
        ...
        'landmark' => cdp_sanitize($_POST['landmark']),
    ...
    $db->bind(':landmark', $datos['landmark']);
    ...
    if (count($_FILES['filesMultiple']['name']) > 0 && $_FILES['filesMultiple']['tmp_name'][0] != '') {
        ...
        foreach ($_FILES["filesMultiple"]['tmp_name'] as $key => $tmp_name) {
            ...
            if(!in_array($key, $deleted_file_ids)) {
                ...
                if ($imageFileZise > 0) {
                    try{
                    if(!move_uploaded_file($_FILES["filesMultiple"]["tmp_name"][$key], $target_file)){
                ...
                cdp_insertDriverFiles($user_created_id, $target_file_url, $image_name, date("Y-m-d H:i:s"), $imageFileType);
            }
        }
    }
